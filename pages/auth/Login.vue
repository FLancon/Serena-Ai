            ...
            <div
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <svg id="logo-54" class="mb-8 w-16 shrink-0 mx-auto"  viewBox="0 0 54 40" fill="none"
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">{{
                            $t('form.welcomeTo') }}
                            {{ appName }} !</div>
                        <span class="text-muted-color font-medium">{{ $t("form.signInToContinue") }}</span>
                    </div>
                    <Message icon="pi pi-exclamation-circle" class="mb-2" v-if="errorMessage" severity="error"><b>{{
                            errorMessage }} </b></Message>
                    <div>
                        <label for="email1"
                            class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">{{
                            $t('form.email') }}</label>
                        <InputText id="email1" type="text" :placeholder="`${t('form.email')}`"
                            class="w-full md:w-[30rem] mb-8" v-model="email" />

                        <label for="password1"
                            class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">{{
                            $t('form.password') }}</label>
                        <Password id="password1" v-model="password" :placeholder="`${t('form.password')}`"
                        ...
                        <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                            ...
                            <span class="font-medium no-underline ml-2 text-right cursor-pointer text-primary">{{
                                $t('form.forgotPassword')}} ?</span>
                        </div>
                        <Button label="Sign In" class="w-full" @click="loginWithSupabase"></Button>
                    </div>
                </div>
            </div>
        </div>
