...
//Rendre dynamique selon les infos remplis par l'user ou non.
const listPbmVisitorsGB = ref(['Code Wifi', 'question oreillers', 'Probleme Wifi', 'Fonctionnement Television', 'Remerciement', 'Restaurant & bar à proximité']);

const listPbmVisitorsSerena = ref(['Probleme Wifi', 'early check-in', 'late check-out', 'nuit supplémentaire', 'Problème technique', 'Manque de consommable', 'Problème de ménage']);
...
export const useAI = () => {
    ...
    async function handleUserNeed(userNeed, propertyDetail, context) {
        ...
        } else if (listPbmVisitorsGB.value.some((item) => userNeed.includes(item))) {
            const response = await openai.chat.completions.create({
                model: 'gpt-4',
                messages: [
                    {
